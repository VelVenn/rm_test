...
int main()
{
    ...
    recorder.set(CAP_PROP_FOURCC, VideoWriter::fourcc('M', 'J', 'P', 'G'));

    VideoWriter video(
        "../res/test.mp4", VideoWriter::fourcc('m', 'p', '4', 'v'),
        30.0, Size(640, 480));
    ...
    createTrackbar(
        ...
        [](int pos, void* userdata) {
            VideoCapture* self = static_cast<VideoCapture*>(userdata); // Cast the userdata to the class type
    ...
    while (true) {
        ...
        fsize << "Frame Size: " << frame.size().width << " * " << frame.size().height;
        fps << "FPS: " << recorder.get(CAP_PROP_FPS);

        putText(frame, fsize.str(), Point(10, 10), FONT_HERSHEY_COMPLEX, 0.4, Scalar(0, 0, 255), 1);
        putText(frame, fps.str(), Point(10, 20), FONT_HERSHEY_COMPLEX, 0.4, Scalar(0, 0, 255), 1);
