...
struct Rect
{
	...
	Rect(
		int id = 1, bool color = 0, int width = 0, int height = 0,
...
class Armor
{
  ...
  public:
	Armor(Rect rect = Rect()) : rect(rect) {}

	void setRect(Rect rect) { this->rect = rect; }

	pair<int, int> getCenter() const  // 获得中心点位置
	{
		return { rect.point.first + rect.width / 2,
				 rect.point.second + rect.height / 2 };
	}
	...
	string getDiagonalWithPrec(int precision = 2)
		const  // 获得对角线长度并保留precision位小数
	{
		stringstream stream;
	...
	vector<pair<int, int>> getVertices() const  // 获得四个顶点坐标
	{
		...
		vertices.push_back({ rect.point.first + rect.width,
							 rect.point.second + rect.height });
