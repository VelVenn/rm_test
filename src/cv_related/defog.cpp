...
Mat toDarkChannel(const Mat& src, int coreSize = 15)
{
	...
	Mat darkChannel;
	erode(
		darkMask, darkChannel,
		getStructuringElement(MORPH_RECT, Size(coreSize, coreSize))
	);
...
Vec3f getAtmLight(
	const Mat& src, const Mat& darkChannel, float threshold = 0.001
)
{
...
Mat getTransmittance(
	const Mat& src, const Vec3f& atmLight, float omega = 0.95, int coreSize = 15
)
{
...
{  // p -> input, i -> guidance
	Mat meanI, meanP, meanIP, meanII, varI,
		covIP;  // mean -> 局部均值, var -> 方差, cov -> 协方差
...
Mat transmittanceRefine(
	const Mat& src, const Mat& initTransmittance, int r = 45,
...
Mat dehaze(
	const Mat& src, const Mat& transmittance, const Vec3f& atmLight,
	const float minTrans = 0.01
)
{
