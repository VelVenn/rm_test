...
class RoiSelector
{
  ...
  private:
	...
	static void callback(int event, int x, int y, int flags, void* userdata)
	{
		...
		RoiSelector* self = static_cast<RoiSelector*>(userdata);
		self->reactor(event, x, y, flags);
	}
	// The callback function for setMouseCallback must be static since its signature is void*,
	// which cannot be cast from a member function pointer
	...
	void reactor(int event, int x, int y, int flags)
	{
		...
		if (x > 0 && y > 0 && x <= img.cols && y <= img.rows) {
			...
			ss << "(" << x << ", " << y << "): "
			   ...
			   << ", B: " << color[0] << " ]";
			putText(
				buffer, ss.str(), Point(10, 20), FONT_HERSHEY_SIMPLEX, 0.5,
				Scalar(0, 0, 255), 1
			);
			...
			case EVENT_MOUSEMOVE:
				if (selecting) {
					...
					roi      = Rect(
						 ...
						 );  // Use min and abs to ensure the ROI is valid dragging in any direction
